...
def build_auction_metrics(auction_raw: pd.DataFrame, all_order_lines: pd.DataFrame, return_lines: pd.DataFrame,
    ...
    auction_lines = filter_by_created_date(auction_lines, start_date, end_date)
    ...
    # Prefer uploaded Returned Order table for detailed return metrics; otherwise use all-order Return/Refund fields.
    ...
    return_count = len(auction_return_orders) if not auction_return_orders.empty else (return_metrics["packages"]["Order IDs"].nunique() if return_metrics else 0)
...
st.success("✅ 版本确认：AUCTION_FINAL_20260511｜Auction 上传入口已启用（页面应看到第 4 个上传框 + Auction Report Tab）")
...
- **Auction Return/Refund**：优先用 Returned Order 表按 Order ID 匹配 Auction 订单做详细退货分析；如果没上传 Returned Order 表，则用 Auction 表里的 `Cancelation/Return Type = Return/Refund` 做申请退货统计。
        """
    )
...
with main_tabs[2]:
    if not auction_ctx.get("has_data"):
        ...
        a3.metric("申请退货（Return/Refund）", fmt_num(auction_ctx["return_orders"]), f"退货率 {auction_ctx['return_rate']:.1f}%")
        a4.metric("直播创建订单", fmt_num(auction_ctx["live_total"]))
        ...
        at = st.tabs(["Auction SKU 分布", "Auction Cancel", "Auction Return", "Auction 明细"])
        with at[0]:
